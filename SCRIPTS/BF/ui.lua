...
local uiState = uiStatus.init --статус, в котором находится программа
...
local function getWeel (min, max)
    local s2Val = getValue('s2') + 1024
    local result = math.floor(s2Val*(max - min + 1)/2048) + 1

    if result > max then result = max end
    return result
end

local function incField()
    currentField = getWeel(1, #Page.fields)
end
...
local function drawScreen()
    ...
    drawScreenTitle("BF / "..Page.title.." field "..getWeel(1,#Page.fields))
end
